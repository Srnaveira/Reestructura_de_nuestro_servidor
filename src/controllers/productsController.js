...
async function getProductById (req, res) {
    try {
        ...
        const productIsValid = await productsService.getProductById(idProduct); 
...
async function updateProduct (req, res) {
    try {
        ...
        await productsService.updateProduct(idProduct, productUpdate);
...
async function getProductByFilter (req, res) {
    ...
    try {
        ...
        const totalProducts = await productsService.totalProducts()

        const totalPages = Math.ceil(totalProducts / limit);
        const offset = (page - 1) * limit;

        const products = await productsService.getProductByFilter(filter, limit, offset, sortOptions)
